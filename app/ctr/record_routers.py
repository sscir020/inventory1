...
def form_search_device():
    ...
    if form.validate_on_submit():
        ...
        if db.session.query(Web_device).filter(Web_device.device_id==device_id).first()!=None:
            ...
            pagination = db.session.query(Opr.opr_id,Material.material_id, Material.material_name,Opr.device_id,Opr.oprtype, Opr.diff, \
            ...
            flash("设备不存在")
    return render_template('search_device.html',form=form,join_oprs=join_oprs,oprenumCH=oprenumCH)
...
def show_join_oprs_main():
    ...
    join_oprs = pagination.items
    db.session.close()
    return render_template('join_oprs_main_table.html',join_oprs=join_oprs,oprenumCH=oprenumCH)
